...
class Species:
    ...
    def addParticle(self,pos,vel,mpw,world,efx,efy,efz):
        ...
        #dt = world.dt
        dcbym = 0.5*self.dt*self.charge/self.mass #preCompute
        vel[0] -= efx_par*dcbym#(0.5*dt*self.charge/self.mass)
        vel[1] -= efy_par*dcbym#(0.5*dt*self.charge/self.mass)            
        vel[2] -= efz_par*dcbym#(0.5*dt*self.charge/self.mass)
    ...
    def advance(self,world):
        ...
        for par in self.particles:
            ...
            par.vel,par.pos = data_utils.advancepar(lc,efx,efy,efz,par.vel,par.pos,world.r_min,world.r_max,dt,dcbym)
            """
            #direct on python
    ...
    def getMOM(self):
        ...
        #--- compute KINETIC ENERGY of the Species---------------
    def getKE(self):
        ke = 0
        for par in self.particles:
            v2 = (par.vel*par.vel).sum()
            ke += par.mpw*v2
        ke *= 0.5*self.mass
